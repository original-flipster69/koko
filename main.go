...
func main() {
	...
	if err := ui.ApplyColors(cfg.Style.ColorScheme); err != nil {
		fmt.Fprintln(os.Stderr, ui.Error(err.Error()))
		os.Exit(1)
	}
	...
	cmdHandlers, knownCommands := cmdHandler(cfg, llm, kokoDir, cfg.Sandbox.Root, playRegistry, src)
	for _, p := range playRegistry.List() {
		knownCommands = append(knownCommands, ":"+p.Name)
	}

	if err := terminal.Run(a, llm.Name(), kokoDir, splashes, cmdHandlers, knownCommands); err != nil {
		fmt.Fprintln(os.Stderr, ui.Error(err.Error()))
		os.Exit(1)
	}
...
func cmdHandler(cfg *config.Config, llm provider.Provider, dataDir string, sandboxRoot string, playRegistry *plays.Registry, src reloadSources) (terminal.CmdHandler, []string) {
	...
	commands = map[string]command{
		...
		":reload": {desc: "reload config from its sources", fn: func(_ string, _ []string, a *agent.Agent) (bool, string, string) {
			...
			if err != nil {
				return true, "", ui.Error(fmt.Sprintf("reload failed (keeping current config): %v", err))
			}
			...
			if len(applied) == 0 && len(restart) == 0 {
				return true, "", ui.Info("reload", "config reloaded — no changes detected")
			}
			var b strings.Builder
			if len(applied) > 0 {
				b.WriteString(ui.Info("applied", strings.Join(applied, ", ")) + "\n")
			}
			if len(restart) > 0 {
				b.WriteString(ui.Info("restart", "changed but needs restart: "+strings.Join(restart, ", ")) + "\n")
			}
