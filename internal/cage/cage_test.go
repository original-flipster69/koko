...
func TestGenerateDarwin(t *testing.T) {
	s, err := Generate(Options{Username: "pushpuppet", GOOS: "darwin"})
	if err != nil {
		t.Fatal(err)
	}
	if s.Filename != "cage-pushpuppet.sh" {
		t.Errorf("filename = %q", s.Filename)
	}
	for _, want := range []string{"dscl", "createhomedir", `NEW_USER="pushpuppet"`, `GROUP="collabo"`, "chmod -R 2770"} {
...
func TestGenerateLinux(t *testing.T) {
	s, err := Generate(Options{Username: "pushpuppet", GOOS: "linux"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerateCustomGroup(t *testing.T) {
	s, err := Generate(Options{Username: "pushpuppet", Group: "devs", GOOS: "linux"})
	if err != nil {
		t.Fatal(err)
	}
	if s.Group != "devs" {
...
func TestGenerateUnsupportedOS(t *testing.T) {
	if _, err := Generate(Options{Username: "pushpuppet", GOOS: "windows"}); err == nil {
...
func TestGenerateRejectsBadGroups(t *testing.T) {
	...
	for _, g := range bad {
		if _, err := Generate(Options{Username: "pushpuppet", Group: g, GOOS: "linux"}); err == nil {
...
func TestGeneratePasswordIsRandomAndShellSafe(t *testing.T) {
	s1, _ := Generate(Options{Username: "pushpuppet", GOOS: "linux"})
	s2, _ := Generate(Options{Username: "pushpuppet", GOOS: "linux"})
	if s1.Body == s2.Body {
