...
func (k koko) do(opts cmdOpts) string {
...
func (c clear) do(opts cmdOpts) string {
	opts.a.ClearHistory()
...
func (u undo) do(opts cmdOpts) string {
	path, err := opts.a.Undo()
	switch {
	case err != nil:
		return opts.scheme().Error(fmt.Sprintf("undo failed: %v", err))
	case path == "":
		return opts.scheme().Info("undo", "nothing to undo")
	default:
		return opts.scheme().Info("undo", fmt.Sprintf("reverted %s", path))
	}
}
...
func (t tokens) do(opts cmdOpts) string {
...
func (c compact) do(opts cmdOpts) string {
	oldTokens, newTokens := opts.a.Compact()
	return opts.scheme().Info("compact", fmt.Sprintf("~%d → ~%d tokens", oldTokens, newTokens))
}
...
func (p plan) do(opts cmdOpts) string {
	if opts.a.TogglePlanMode() {
		return opts.scheme().Info("plan", "mode on — read-only; call :plan again to exit")
	}
	return opts.scheme().Info("plan", "mode off — full tools restored")
}
...
func (h help) do(_ cmdOpts) string {
